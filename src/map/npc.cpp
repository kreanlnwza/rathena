...
const std::string BarterCraftDatabase::getDefaultLocation(){
    return std::string(db_path) + "/barter_craft_db.yml";
}
...
uint64 BarterCraftDatabase::parseBodyNode( const ryml::NodeRef& node ){
    uint64 count = 0;

    for( const ryml::NodeRef& bodyNode : node ){
        if( !bodyNode.has_child( "Item" ) ){
            this->invalidWarning( bodyNode, "barter_craft_parseBodyNode: Missing Item entry.\n" );
            continue;
        }

        std::string item_name;

        if( !this->asString( bodyNode, "Item", item_name ) ){
            continue;
        }

        std::shared_ptr<item_data> target_item = item_db.search_aegisname( item_name.c_str() );

        if( target_item == nullptr ){
            this->invalidWarning( bodyNode["Item"], "barter_craft_parseBodyNode: Unknown item %s.\n", item_name.c_str() );
            continue;
        }

        std::shared_ptr<s_npc_barter_craft> craft = std::make_shared<s_npc_barter_craft>();
        craft->name = item_name;
        craft->npcid = 0;
        craft->m = -1;
        craft->x = 0;
        craft->y = 0;
        craft->dir = (uint8)DIR_NORTH;
        craft->sprite = JT_FAKENPC;

        std::shared_ptr<s_npc_barter_craft_item> entry = std::make_shared<s_npc_barter_craft_item>();
        entry->index = 0;
        entry->nameid = target_item->nameid;
        entry->stockLimited = false;
        entry->stock = 0;
        entry->price = 0;
        entry->refine = 0;
        entry->rate = 100;

        if( bodyNode.has_child( "Rate" ) ){
            uint16 rate;

            if( !this->asUInt16( bodyNode, "Rate", rate ) ){
                continue;
            }

            if( rate > 100 ){
                this->invalidWarning( bodyNode["Rate"], "barter_craft_parseBodyNode: Rate %hu out of range (0-100), capping.\n", rate );
                rate = 100;
            }
        ...
        if( bodyNode.has_child( "Cost" ) ){
            for( const ryml::NodeRef& costNode : bodyNode["Cost"] ){
                if( entry->requirements.size() >= MAX_BARTER_REQUIREMENTS ){
                    this->invalidWarning( costNode, "barter_craft_parseBodyNode: Too many requirements, maximum %d.\n", MAX_BARTER_REQUIREMENTS );
                    break;
                }

                if( !costNode.has_child( "Item" ) ){
                    this->invalidWarning( costNode, "barter_craft_parseBodyNode: Missing Item in cost entry.\n" );
                    continue;
                }

                std::string cost_name;

                if( !this->asString( costNode, "Item", cost_name ) ){
                    continue;
                }

                std::shared_ptr<item_data> cost_item = item_db.search_aegisname( cost_name.c_str() );

                if( cost_item == nullptr ){
                    this->invalidWarning( costNode["Item"], "barter_craft_parseBodyNode: Unknown cost item %s.\n", cost_name.c_str() );
                    continue;
                }

                uint16 index = (uint16)entry->requirements.size();
                std::shared_ptr<s_npc_barter_craft_requirement> requirement = std::make_shared<s_npc_barter_craft_requirement>();
                requirement->index = index;
                requirement->nameid = cost_item->nameid;
                requirement->refine = -1;

                uint16 amount = 1;

                if( costNode.has_child( "Amount" ) ){
                    if( !this->asUInt16( costNode, "Amount", amount ) ){
                        continue;
                    }

                    if( amount > MAX_AMOUNT ){
                        this->invalidWarning( costNode["Amount"], "barter_craft_parseBodyNode: Amount %hu too large, capping to %hu.\n", amount, MAX_AMOUNT );
                        amount = MAX_AMOUNT;
                    }
                }
                ...
                entry->requirements[index] = requirement;
            }
        }

        craft->items[0] = entry;

        this->put( craft->name, craft );
        count++;
    }
...
void BarterCraftDatabase::loadingFinished(){
    TypesafeYamlDatabase::loadingFinished();
}

s_npc_barter_craft::~s_npc_barter_craft(){
    if( this->npcid != 0 ){
        npc_data* nd = map_id2nd( this->npcid );
...
BarterCraftDatabase barter_craft_db;
...
e_purchase_result npc_barter_craft_purchase( map_session_data& sd, std::shared_ptr<s_npc_barter_craft_item> item ){
    if( item == nullptr ){
        return e_purchase_result::PURCHASE_FAIL_EXCHANGE_FAILED;
    }

    struct item_data* data = item_db.find( item->nameid ).get();

    if( data == nullptr ){
        return e_purchase_result::PURCHASE_FAIL_EXCHANGE_FAILED;
    }

    // Check for required weight limit and slots
    char add_result = pc_checkadditem( &sd, item->nameid, 1 );

    if( add_result == CHKADDITEM_OVERAMOUNT ){
        return e_purchase_result::PURCHASE_FAIL_COUNT;
    }

    if( add_result == CHKADDITEM_NEW ){
        if( pc_inventoryblank( &sd ) < data->inventorySlotNeeded( 1 ) ){
            return e_purchase_result::PURCHASE_FAIL_COUNT;
        }
    }

    if( sd.weight + data->weight > sd.max_weight ){
        return e_purchase_result::PURCHASE_FAIL_WEIGHT;
    }
    ...
    for( const auto& requirementPair : item->requirements ){
        std::shared_ptr<s_npc_barter_craft_requirement> requirement = requirementPair.second;

        if( requirement == nullptr ){
            return e_purchase_result::PURCHASE_FAIL_EXCHANGE_FAILED;
        }

        uint32 total_needed = requirement->amount;

        while( total_needed > 0 ){
            int32 inventory_index = pc_search_inventory( &sd, requirement->nameid );

            if( inventory_index == -1 ){
                return e_purchase_result::PURCHASE_FAIL_GOODS;
            }

            if( sd.inventory.u.items_inventory[inventory_index].equip != 0 ){
                return e_purchase_result::PURCHASE_FAIL_GOODS;
            }

            if( battle_config.hide_fav_sell && sd.inventory.u.items_inventory[inventory_index].favorite != 0 ){
                return e_purchase_result::PURCHASE_FAIL_GOODS;
            }

            uint32 available = sd.inventory.u.items_inventory[inventory_index].amount - requiredItems[inventory_index];

            if( available == 0 ){
                return e_purchase_result::PURCHASE_FAIL_GOODS;
            }

            uint32 take = std::min( available, total_needed );
            requiredItems[inventory_index] += take;
            total_needed -= take;
        }
    }
    ...
    for( int32 i = 0; i < MAX_INVENTORY; ++i ){
        if( requiredItems[i] > 0 ){
            if( pc_delitem( &sd, i, requiredItems[i], DELITEM_ALLOW_REFINE, 0, LOG_TYPE_BARTER ) != 0 ){
                return e_purchase_result::PURCHASE_FAIL_EXCHANGE_FAILED;
            }
        }
    }
    ...
    // Craft success check
    uint16 rate = item->rate;

    if( rate >= 100 || rnd()%100 < rate ){
        struct item crafted_item = {};
        crafted_item.nameid = item->nameid;
        crafted_item.identify = true;

        if( pc_additem( &sd, &crafted_item, 1, LOG_TYPE_BARTER ) != ADDITEM_SUCCESS ){
            return e_purchase_result::PURCHASE_FAIL_EXCHANGE_FAILED;
        }
    ...
    // Failure case: materials already consumed, no item granted
    return e_purchase_result::PURCHASE_FAIL_EXCHANGE_FAILED;
}
