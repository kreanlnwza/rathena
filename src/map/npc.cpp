...
/**
 * Reload NPCs only (keep monsters alive)
 * Used by @reloadscriptnomobs
 */
int32 npc_reload_nomobs(void) {
    int32 npc_new_min = npc_id;
    struct s_mapiterator* iter;
    struct block_list* bl;
    ...
    for( bl = (struct block_list*)mapit_first(iter); mapit_exists(iter); bl = (struct block_list*)mapit_next(iter) ) {
        ...
        case BL_NPC:
            if( bl->id != fake_nd->id )// don't remove fake_nd
                npc_unload((struct npc_data *)bl, false);
            break;
        // Skip BL_MOB - keep monsters alive
        }
    }
    mapit_free(iter);
    ...
    if( battle_config.dynamic_mobs ){
        for (int32 i = 0; i < map_num; i++) {
            // Skip mob cleanup - keep monsters
            struct map_data *mapdata = map_getmapdata(i);
            
            if( mapdata->npc_num > 0 ){
                ShowWarning( "npc_reload_nomobs: %d npcs weren't removed at map %s!\n", mapdata->npc_num, mapdata->name );
            }
        }
    }

    // Don't clear mob spawn lookup index - keep monster spawn info
    // mob_clear_spawninfo();

    npc_warp = npc_shop = npc_script = 0;
    // Keep mob counts - don't reset
    // npc_mob = npc_cache_mob = npc_delay_mob = 0;
