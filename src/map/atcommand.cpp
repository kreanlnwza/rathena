...
/*==========================================
 * @reloadmonsters - Reload only monster spawns
 *------------------------------------------*/
ACMD_FUNC(reloadmonsters){
    ...
    // Close any open NPC dialogs for all players to prevent issues
    iter = mapit_getallusers();
    for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) ){
        pc_close_npc(pl_sd,1);
    }
    ...
    // Remove all existing monsters (BL_MOB) from all maps
    ...
    // Reset monster-related counters (important for proper statistics)
    ...
    // Load common monster spawn files (loaded in all modes)
    npc_parsesrcfile("npc/mobs/jail.txt");
    npc_parsesrcfile("npc/mobs/pvp.txt");
    npc_parsesrcfile("npc/mobs/towns.txt");

    // Load mode-specific monster spawn files
#ifdef RENEWAL
    clif_displaymessage(fd, "Loading Renewal monster spawns...");
    npc_parsesrcfile("npc/re/scripts_monsters.conf");
#else
    clif_displaymessage(fd, "Loading Pre-Renewal monster spawns...");
    npc_parsesrcfile("npc/pre-re/scripts_monsters.conf");
#endif

    // Rebuild NPC events cache (important for monster-related events)
