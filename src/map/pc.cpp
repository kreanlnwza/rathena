...
/*==========================================
 * Update player's inventory slots and handle item management
 * @param sd: Player
 * @param new_slots: New number of inventory slots
 * @return True on success, False on failure
 *------------------------------------------*/
bool pc_update_inventory_slots(map_session_data *sd, uint16 new_slots) {
    nullpo_retr(false, sd);
    
    // Validate new slots value
    if (new_slots < INVENTORY_BASE_SIZE || new_slots > MAX_INVENTORY) {
        ShowError("pc_update_inventory_slots: Invalid slot count %d for player %s (account_id=%d, char_id=%d)\n", 
            new_slots, sd->status.name, sd->status.account_id, sd->status.char_id);
        return false;
    }

    uint16 old_slots = sd->status.inventory_slots;
    
    // If slots are decreasing, need to handle excess items
    if (new_slots < old_slots) {
        // Count items in slots that will be removed
        int32 items_to_remove = 0;
        for (uint16 i = new_slots; i < old_slots; i++) {
            if (sd->inventory.u.items_inventory[i].nameid != 0) {
                items_to_remove++;
            }
        }
        
        // If there are items in slots that will be removed, we can't reduce slots
        if (items_to_remove > 0) {
            ShowInfo("pc_update_inventory_slots: Cannot reduce slots for player %s - %d items would be lost\n", 
                sd->status.name, items_to_remove);
            return false;
        }
    }

    // Update the slot count
    sd->status.inventory_slots = new_slots;
    
    // Save the character data
    chrif_save(sd, CSAVE_NORMAL);
    
    // Update client
    clif_updatestatus(*sd, SP_INVENTORYSLOTS);
    
    ShowInfo("pc_update_inventory_slots: Updated slots for player %s from %d to %d\n", 
        sd->status.name, old_slots, new_slots);
