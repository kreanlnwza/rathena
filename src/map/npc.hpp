...
struct s_npc_barter_craft_requirement{
    uint16 index;
    t_itemid nameid;
    uint16 amount;
    int8 refine;
...
struct s_npc_barter_craft_item{
    uint16 index;
    t_itemid nameid;
    uint16 rate;
    bool stockLimited;
    ...
    uint32 price;
    int8 refine;
    std::map<uint16, std::shared_ptr<s_npc_barter_craft_requirement>> requirements;
};

struct s_npc_barter_craft{
    std::string name;
    int16 m;
    uint16 x;
    uint16 y;
    uint8 dir;
    int16 sprite;
    std::map<uint16, std::shared_ptr<s_npc_barter_craft_item>> items;
    int32 npcid;

    ~s_npc_barter_craft();
};

class BarterCraftDatabase : public TypesafeYamlDatabase<std::string, s_npc_barter_craft>{
public:
    BarterCraftDatabase() : TypesafeYamlDatabase( "BARTER_CRAFT_DB", 1, 1 ){
...
extern BarterCraftDatabase barter_craft_db;

struct s_barter_craft_purchase{
    std::shared_ptr<s_npc_barter_craft_item> item;
    uint32 amount;
    item_data* data;
};
