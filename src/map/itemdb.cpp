...
/**
* Reload a specific Item DB file
* @param filepath: Path to the YAML file to reload
* @return true on success, false on failure
*/
bool itemdb_reload_file(const char* filepath) {
    struct s_mapiterator* iter;
    map_session_data* sd;

    if (!filepath || !*filepath) {
        ShowError("itemdb_reload_file: Invalid filepath provided.\n");
        return false;
    }
    ...
    try {
        tree = parser.parse_in_arena(c4::to_csubstr(filepath), c4::to_csubstr(buf));
    } catch(const std::runtime_error& e) {
        ShowError("itemdb_reload_file: Failed to parse YAML file '%s'.\n", filepath);
        ShowError("Error message: %s\n", e.what());
        aFree(buf);
        return false;
    }

    if (!tree.rootref().has_child(c4::to_csubstr("Header"))) {
        ShowError("itemdb_reload_file: File '%s' is missing a Header node.\n", filepath);
        aFree(buf);
        return false;
    }
    ...
    if (header.has_child(c4::to_csubstr("Type"))) {
        header["Type"] >> typeStr;
        if (typeStr != "ITEM_DB") {
            ShowError("itemdb_reload_file: File '%s' is not an ITEM_DB file (Type: %s).\n", filepath, typeStr.c_str());
            aFree(buf);
            return false;
        }
    } else {
        ShowError("itemdb_reload_file: File '%s' Header is missing Type field.\n", filepath);
        aFree(buf);
        return false;
    }

    if (tree.rootref().has_child(c4::to_csubstr("Body"))) {
        const ryml::NodeRef& bodyNode = tree["Body"];
        uint64 count = 0;

        for (const ryml::NodeRef &node : bodyNode) {
            count += item_db.parseBodyNode(node);
        }

        ShowStatus("Reloaded '%" PRIu64 "' entries from '%s'\n", count, filepath);
    }
